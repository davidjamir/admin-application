...
export function useTokenIngestion(
  adminPassword: string,
  isAdminVerified: boolean,
) {
  const [status, setStatus] = useState(
    "Select Business Manager to begin discovery.",
  );
  ...
  const bmFilterOptions = useMemo(() => {
    ...
    return systemUsers
      .map((u) => ({
        id: (u.businessId ?? "").trim(),
        name: (u.businessName ?? "—").trim() || "—",
      }))
  ...
  const filteredSystemUsers = useMemo(() => {
    ...
    return systemUsers.filter(
      (u) => (u.businessId ?? "").trim() === selectedBmFilter,
    );
  }, [systemUsers, selectedBmFilter]);

  const selectedUser = useMemo(
    () => systemUsers.find((u) => u.id === selectedSystemUserId),
    [systemUsers, selectedSystemUserId],
  );
  ...
  const activePart = useMemo(() => {
    ...
    for (const part of [1, 2, 3] as const) {
      ...
      if (
        selectedPageIds.length === partIds.length &&
        partIds.every((id) => selectedSet.has(id))
      ) {
        return part;
      }
    }
      ...
      try {
        ...
        if (json.data) {
          const topics = Array.from(
            new Set(
              json.data.map((p: MongoPageData) => p.topic).filter(Boolean),
            ),
          ) as string[];
  ...
  const handlePageSave = async () => {
    ...
    try {
      ...
      setStatus(
        `Ingestion finalized: ${selectedPages.length} tokens committed.`,
      );
      setSelectedPageIds([]);
    } catch {
